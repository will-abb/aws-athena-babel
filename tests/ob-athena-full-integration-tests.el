...
(ert-deftest ob-athena-user-profiles-filter-by-name-score-signup ()
  ...
  (let* ((timestamp (floor (float-time)))
         ...
         (csv-dir (expand-file-name (number-to-string timestamp) temporary-file-directory))
         (org-src (format
                   "#+begin_src athena :aws-profile \"williseed-athena\" :database \"default\" :s3-output-location \"%s\" :csv-output-dir \"%s\" :workgroup \"primary\" :poll-interval 3 :fullscreen nil :result-reuse-enabled nil :result-reuse-max-age 10080 :console-region \"us-east-1\" :var name=\"Alice\" score=90 signup_date=\"2024-01-01\"\nSELECT id, name, score, signup_date FROM test_user_profiles\nWHERE name != '${name}'\nAND score >= ${score}\nAND signup_date != '${signup_date}';\n#+end_src"
                   s3-path csv-dir)))
    (make-directory csv-dir t)
    (with-temp-buffer
      (insert org-src)
